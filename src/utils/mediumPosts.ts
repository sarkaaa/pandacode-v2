...
const profileUrl = "https://medium.com/@sarkachwastkova";
...
const articleElements = document.querySelectorAll("article") as any;

const articles = await Promise.all(
	articleElements.map(async (element: HTMLElement) => {
...
fs.writeFile(`${dataDir}/articles.json`, JSON.stringify(articles, null, 2))
	...
	.catch((error: Error) => console.error("Error saving articles:", error));
